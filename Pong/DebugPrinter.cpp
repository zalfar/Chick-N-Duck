...
void COLORTORGBA(int col, unsigned char& r, unsigned char& g, unsigned char& b, unsigned char& a) //warna dari score
{
	r = (col & 0x00ff0000) >> 16; 
	g = (col & 0x0000ff00) >> 8;
	b = (col & 0x000000ff);
...
void DebugPrinter::Init(SDL_Renderer* renderer)
{
	...
	if (image.Load("resources/font.bmp") != false) //untuk memasukkan font untuk score
	{
...
void DebugPrinter::SetScale(float w, float h) //ukuran font
{
	fontScaleX = w;
	fontScaleY = h;
}

void DebugPrinter::SetJustification(Justification just) //posisi font ditengah layar
{
	currentJust = just;
}
...
void DebugPrinter::Print(SDL_Renderer* renderer, int x, int y, int colour, char* fmt, ...) //menampilkan score dan mekanisma scoring
{
	char buffer[4096];
